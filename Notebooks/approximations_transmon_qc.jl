...
# ╔═╡ 3a24b105-a487-4b26-ace4-2b637698a113
...
begin
	...
	a(i) = IndexedOperator(Transition(h,:σ,:g,:e), i)

end

# ╔═╡ 743fde4d-ca25-4fa8-9c6b-c3b3a77e148d
md"
**Do not** use imaginary unity $i$ in Hamiltonians, use instead two parameters for the conjugate one and for the non-conjugate. Otherwise, the derived equations may be **wrong**.
"
...
begin
	...
	H_q = ∑((w(i) - w_ext)*a(i)'*a(i) - 0.5*A(i)*a(i)'*a(i)'*a(i)*a(i), i)
	H_f = ∑(g(i)*a(i) + gc(i)*a(i)', i)
	H_i = ∑(Ω(i,j)*a(i)'*a(j), j, i)
	H = H_q + H_f + H_i
...
# ╔═╡ f84778f0-0c6d-4ae2-b9ad-5afbf310d7a1
me_comp

# ╔═╡ 301c0274-80d5-4f02-954b-f714f9de1b2d
# Generate parameters
run(`bash run_circ_parameters.sh 3.3001e-9 3.3e-9`)
...
begin
	...
	# list of parameters
	ps = [wi; gi; gci; Ai; Γij; Ωij; w_ext; g_nr; g_phi]
	pn = collect(Iterators.flatten([w_, g_, conj.(g_), A_, Γ_, Ω_, w_ext_, g_nr_, g_phi_]))
	p0 = ps .=> pn
	tend = 6.45e-2#1e0/minimum(eigen(Γ_).values)
...
# ╔═╡ 615574b4-a7df-4743-a726-ad6fa828f1ee
SteadyStateProblem(prob)

# ╔═╡ 9043051b-e185-4b60-ad9a-cf725744ec1d
solve(NonlinearProblem(SteadyStateProblem(prob)), NewtonRaphson(autodiff=false))

# ╔═╡ 3e17ba92-bdb9-41e6-be77-9f5895ac95af
sol[end]

# ╔═╡ 72141267-7788-4910-90d5-36bb9796e04d
minimum(eigen(Γ_).values)

# ╔═╡ 2f5aefb2-780c-462c-b896-75826450fdb5
begin
	
	# Plot
	p1 = plot(sol.t, real.(sol[a(1)] .+ conj.(sol[a(1)])), linewidth=2, xlabel="Time", ylabel="Dipole moment", label="qubit 1", legend = true)
	plot!(sol.t, real.(sol[a(2)] .+ conj.(sol[a(2)])), linewidth=2, xlabel="Time", ylabel="Dipole moment", label="qubit 2", legend = true)
	p2 = plot(sol.t, abs.((a_inc["R"] .+ sum(exp(-im*w_[j]*t_j_[j]) * 
			 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))), linewidth=2, xlabel="Time", ylabel="Output", label="R", legend = true)
	plot!(sol.t, abs.((a_inc["L"] .+ sum(exp(im*w_[j]*t_j_[j]) * 
			 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))), linewidth=2, xlabel="Time", ylabel="Output", label="L", legend = true)
	plot!(sol.t, [a_inc[DIRECTION] for j=1:length(sol.t)], linestyle=:dash, color=:black, label="a_in", legend=true)
...
function transmission_power_dependence(a_in; direction="R")
	...
	# solving ODEs
	prob = ODEProblem(sys,u0,(0.0,tend),p0)
	sol = solve(prob, Kvaerno5(autodiff=false), reltol=1e-8, abstol=1e-8, maxiters=2e6, saveat=tlist)
	...
	if length(me_comp) > 6
		...
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(-im*(w_[2]*t_j_[2] - w_[1]*t_j_[1]))*conj.(sol[a(1)*a(2)']) .+
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(-im*(w_[1]*t_j_[1] - w_[2]*t_j_[2]))*sol[a(1)*a(2)']
		)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "4c922703e3936c72a754fd7bd8d59cb675501706"
