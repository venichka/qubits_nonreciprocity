...
# ╔═╡ 2a357850-4ead-11ee-1bb7-4370e19f7051
begin
	using QuantumOptics
	using ModelingToolkit, OrdinaryDiffEq, LinearAlgebra, Statistics
	using QuantumCumulants
	using Plots, LaTeXStrings
	using HDF5, FileIO
end

# ╔═╡ e1b502b2-20c7-4564-94ca-eeeb3dd2e232
begin
	# Parameters
	...
	# Hilbertspace
	# h = FockSpace(:qubit)
	# h = ⊗([FockSpace(Symbol(:qubit, i)) for i=1:N_qubits]...)
	h = ⊗([NLevelSpace(Symbol(:qubit, i),(:g,:e)) for i=1:N_qubits]...)

	# i = Index(h, :i, N, h)
	# j = Index(h, :j, N, h)
	
	w_ext, g_nr, g_phi = cnumbers("ωₑₓₜ γₙᵣ γₚₕ")
	w(i) = IndexedVariable(:ω, i)
	A(i) = IndexedVariable(:A, i)
	Γ(i,j) = IndexedVariable(:Γ, i, j)
	Ω(i,j) = IndexedVariable(:Ω, i, j)
	g(i) = IndexedVariable(:g, i)
	g_r(i) = IndexedVariable(:gʳ, i)
	g_i(i) = IndexedVariable(:gⁱ, i)

	# w = IndexedVariable(:ω, i)
	# A = IndexedVariable(:A, i)
	# Γ = IndexedVariable(:Γ, i, j)
	# Ω = IndexedVariable(:Ω, i, j)
	# g = IndexedVariable(:g, i)
	# g_r = IndexedVariable(:gʳ, i)
	# g_i = IndexedVariable(:gⁱ, i)
	
	# Operators
	# a(i) = IndexedOperator(Destroy(h, :a, i), i)
	a(i) = IndexedOperator(Transition(h,:σ,:g,:e, i), i)

end

# ╔═╡ aea99bd1-9d9b-4a67-8ecd-af773a895e49
begin
	# Hamiltonian
	H_q = sum((w(i) - w_ext)*a(i)'*a(i) - A(i)/2*a(i)'*a(i)'*a(i)*a(i) 
			  for i=1:N_qubits)
	H_f = sum(g(i)*a(i) + conj(g(i))*a(i)' for i=1:N_qubits)
	H_i = sum(Ω(i,j)*a(i)'*a(j) for i=1:N_qubits, j=1:N_qubits)
	H = H_q + H_f + H_i

	# H_q = ∑((w - w_ext)*a(i)'*a(i) - 0.5*A*a(i)'*a(i)'*a(i)*a(i), i)
	# H_f = ∑(g*a(i) + conj(g)*a(i)', i)
	# H_i = ∑(Ω*a(i)'*a(j), i, j)
	# H = H_q + H_f + H_i
	
	# Jumps
	J = [a(i) for i=1:N_qubits]
	
	# Rates
	rates = [Γ(i,j) for i=1:N_qubits, j=1:N_qubits]
end

# ╔═╡ 961500d8-a34f-4880-b596-f5b6f1c53435
H

# ╔═╡ a144b60b-ae40-4499-8be3-7a011f76d5a8
begin
	# list of operators
	n_order = 2
	ops = (n_order > 1) ? [a(1), a(2), a(1)'*a(2), a(2)'*a(1)] : [a(1), a(2)]
	
	eqs = meanfield(ops, H, J; rates=rates)
	eqs_expanded = cumulant_expansion(eqs, n_order) #n-order average
end

# ╔═╡ 967b6316-ea14-4f0b-b7b8-6e6c99e80881
me_comp = complete(eqs_expanded) #automatically complete the system

# ╔═╡ fde0a36b-f9c4-455c-a8d3-a136cc77bea6
...
# ╔═╡ 301c0274-80d5-4f02-954b-f714f9de1b2d
# Generate parameters
run(`bash run_circ_parameters.sh 3.31e-9 3.3e-9`)

# ╔═╡ 4c9cb6fb-0c81-4f03-89da-29d28b48f0d2
# Load parameters
params = load("../Data/params.h5")

# ╔═╡ 2e359f7a-a6f5-4393-bf23-7e988ebd8bcb
# Calculating missing parameters
begin
	DIRECTION = "L"
	a_mag = 0.001*sqrt(1.5e9)
	...
	wi = [w(i) for i=1:N_qubits]
	gi = [g(i) for i=1:N_qubits]
	Ai = [A(i) for i=1:N_qubits]
	Γij = [Γ(i,j) for i = 1:N_qubits for j=1:N_qubits]
	Ωij = [Ω(i,j) for i = 1:N_qubits for j=1:N_qubits]
end

# ╔═╡ 2f39dded-4aad-4288-b5c6-4447dd035729
begin
	# initial state
	u0 = zeros(ComplexF64, length(me_comp))

	# list of parameters
	ps = [wi; gi; Ai; Γij; Ωij; w_ext; g_nr; g_phi]
	pn = collect(Iterators.flatten([w_, g_, A_, Γ_, Ω_, w_ext_, g_nr_, g_phi_]))
	p0 = ps .=> pn
	...
	sol = solve(prob, Kvaerno5(autodiff=false), reltol=1e-8, abstol=1e-8, saveat=tlist)
end

# ╔═╡ d12cf3bf-7f62-434a-ad02-10a831a21884
p0

# ╔═╡ 920bb3fa-d535-4c7d-b0db-bae00fcd818c
sol.t

# ╔═╡ 2f5aefb2-780c-462c-b896-75826450fdb5
begin
	
	# Plot
	p1 = plot(sol.t*γ_D, real.(sol[a(1)] .+ conj.(sol[a(1)])), linewidth=2, xlabel="Time", ylabel="Dipole moment", label="qubit 1", legend = true)
	plot!(sol.t*γ_D, real.(sol[a(2)] .+ conj.(sol[a(2)])), linewidth=2, xlabel="Time", ylabel="Dipole moment", label="qubit 2", legend = true)
	p2 = plot(sol.t*γ_D, abs.((a_inc["R"] .+ sum(exp(-im*w_[j]*t_j_[j]) * 
			 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))), linewidth=2, xlabel="Time", ylabel="Output", label="R", legend = true)
	plot!(sol.t*γ_D, abs.((a_inc["L"] .+ sum(exp(im*w_[j]*t_j_[j]) * 
			 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))), linewidth=2, xlabel="Time", ylabel="Output", label="L", legend = true)
	plot!(sol.t*γ_D, [a_inc[DIRECTION] for j=1:length(sol.t)], linestyle=:dash, color=:black, label="a_in", legend=true)
	plot(p1, p2, layout=(2,1))
end

# ╔═╡ 6390df00-bce6-4403-bef0-70f9ce581a57
let
	p = plot(sol.t*γ_D, real.(sol[a(1)*a(2)]), linewidth=2, xlabel="Time", ylabel="Correlation", legend = true)
	plot!(sol.t*γ_D, real.(sol[a(1)*a(2)'*a(2)]), linewidth=2)
	plot!(sol.t*γ_D, real.(sol[a(2)*a(1)'*a(1)]), linewidth=2)
	plot!(sol.t*γ_D, real.(sol[a(1)'*a(2)]), linewidth=2)
end

# ╔═╡ 88c00099-85a6-42d9-afc5-d5f7ac1e54e7
# Correlation function and spectra

begin
	out_op_R = a_inc["R"] + sum(sqrt(0.5*params["gammas"][j])*exp(-im*w_[j]*t_j_[j])*a(j) for j=1:N_qubits)
	out_op_L = a_inc["L"] + sum(sqrt(0.5*params["gammas"][j])*exp(im*w_[j]*t_j_[j])*a(j) for j=1:N_qubits)
	out_op = Dict("R" => out_op_R, "L" => out_op_L)
end

# ╔═╡ 9b797ce5-97cf-49fa-a663-6707ebc33a7f
# Power dependence

function transmission_power_dependence(a_in; direction="R")
    a_inc = Dict("L" => direction == "L" ? a_in : 0.0,
				 "R" => direction == "R" ? a_in : 0.0)
	g_ = 1.0im*sqrt.(0.5*params["gammas"]).*(a_inc["R"] * exp.(-im*w_ext_*t_j_) .+
											 a_inc["L"] * exp.(im*w_ext_*t_j_))
	...
	pn = collect(Iterators.flatten([w_, g_, A_, Γ_, Ω_, w_ext_, g_nr_, g_phi_]))
	p0 = ps .=> pn
	tend = 5e0/minimum(eigen(Γ_).values)
	...
	# output operators
	if length(me_comp) > 6
		out2_R = (abs(a_inc["R"])^2 .+ 
		0.5*params["gammas"][1]*sol[a(1)'*a(1)] .+ 
		0.5*params["gammas"][2]*sol[a(2)'*a(2)] .+
		conj(a_inc["R"])*exp(-im*w_[1]*t_j_[1])*sqrt(0.5*params["gammas"][1])*sol[a(1)] .+ 
		conj(a_inc["R"])*exp(-im*w_[2]*t_j_[2])*sqrt(0.5*params["gammas"][2])*sol[a(2)] .+ 
		a_inc["R"]*exp(im*w_[1]*t_j_[1])*sqrt(0.5*params["gammas"][1])*sol[a(1)'] .+ 
		a_inc["R"]*exp(im*w_[2]*t_j_[2])*sqrt(0.5*params["gammas"][2])*sol[a(2)'] .+ 
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(-im*(w_[2]*t_j_[2] - w_[1]*t_j_[1]))*sol[a(1)'*a(2)] .+
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(-im*(w_[1]*t_j_[1] - w_[2]*t_j_[2]))*conj.(sol[a(1)'*a(2)])#sol[a(1)*a(2)']
		)
		out2_L = (abs(a_inc["L"])^2 .+ 
		0.5*params["gammas"][1]*sol[a(1)'*a(1)] .+ 
		0.5*params["gammas"][2]*sol[a(2)'*a(2)] .+
		conj(a_inc["L"])*exp(im*w_[1]*t_j_[1])*sqrt(0.5*params["gammas"][1])*sol[a(1)] .+ 
		conj(a_inc["L"])*exp(im*w_[2]*t_j_[2])*sqrt(0.5*params["gammas"][2])*sol[a(2)] .+ 
		a_inc["L"]*exp(-im*w_[1]*t_j_[1])*sqrt(0.5*params["gammas"][1])*sol[a(1)'] .+ 
		a_inc["L"]*exp(-im*w_[2]*t_j_[2])*sqrt(0.5*params["gammas"][2])*sol[a(2)'] .+ 
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(im*(w_[2]*t_j_[2] - w_[1]*t_j_[1]))*sol[a(1)'*a(2)] .+
		sqrt(0.5*params["gammas"][1])*sqrt(0.5*params["gammas"][2])*exp(im*(w_[1]*t_j_[1] - w_[2]*t_j_[2]))*conj.(sol[a(1)'*a(2)])#sol[a(1)*a(2)']
		)
		out2 = Dict("R" => out2_R, "L" => out2_L)
		return abs.(out2[direction][end]) / a_inc[direction].^2
	else
		out_R = (a_inc["R"] .+ sum(exp(-im*w_[j]*t_j_[j]) * 
				 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))
		out_L = (a_inc["L"] .+ sum(exp(im*w_[j]*t_j_[j]) * 
				 sqrt(0.5*params["gammas"][j]) * sol[a(j)] for j=1:N_qubits))
		out = Dict("R" => out_R, "L" => out_L)
		return abs.(out[direction][end] / a_inc[direction]).^2	
	end
end

# ╔═╡ c3bda9ce-36db-4b69-b1ab-7943dd9c33f4
transmission_power_dependence(100*a_mag; direction="R")

# ╔═╡ 041c919c-32ef-4600-b076-c01277e32daa
# Power dependence computing

begin
	NMAX = 50
	transmissions = zeros(NMAX, 2)
	a_in_list = 10.0.^range(-4, 1, NMAX).*sqrt(1.5e9)
	for iikk in CartesianIndices((NMAX, 2))
		(ii, kk) = Tuple(iikk)[1], Tuple(iikk)[2]
		DIRECTION = (kk == 1) ? "R" : "L"
		a_mag = a_in_list[ii]
		transmissions[ii, kk] = transmission_power_dependence(a_mag; direction=DIRECTION)
	end
end

# ╔═╡ fb1b9084-0ee7-471d-aa46-b6441ba0758b
plot(a_in_list.^2 ./ mean(params["gammas"]), transmissions, xscale=:log10, linewidth=2, label=["R" "L"], xlabel=L"|a_\mathrm{inc}|^2 / \bar{\gamma}", ylabel="Transmission")

# ╔═╡ 53e391b7-8101-480b-91c1-b9999f44393f
transmissions[:, 2]

# ╔═╡ 56e076e0-fbd2-4000-bf30-1b8b9b790517
sol[a(2)'*a(2)]

# ╔═╡ 37254963-13aa-4f11-a967-31102b19606a
md"
## Write to file
"

# ╔═╡ db75e984-c94b-468c-8f21-d2bcebedfa17
if n_order > 1
	data = Dict("a(1)" => sol[a(1)], "a(2)" => sol[a(2)], 
				"a(1)a(2)" => sol[a(1)*a(2)],
				"a(1)'a(2)" => sol[a(1)'*a(2)], "a(1)'a(1)" => sol[a(1)'*a(1)],
				"a(2)'a(2)" => sol[a(2)'*a(2)], 
				"a(2)a(1)'a(1)" => sol[a(2)*a(1)'*a(1)],
				"a(1)a(2)'a(2)" => sol[a(1)*a(2)'*a(2)],
				"transmission_L" => transmissions[:, 2], 
				"transmission_R" => transmissions[:, 1],
				"t_list" => sol.t,
				"a_in_list" => a_in_list, "gammas" => params["gammas"],
				"delta" => params["delta"])
else
	data = Dict("a(1)" => sol[a(1)], "a(2)" => sol[a(2)], 
				"a(1)'a(1)" => sol[a(1)'*a(1)],
...
# ╔═╡ 00a9e7aa-c638-47f7-8452-ded62cff257b
round(params["delta"], digits=4)

# ╔═╡ 7caee069-cfd0-46cc-a643-6f7983da8077
round(a_mag^2 / mean(params["gammas"]), digits=6)
...
Plots = "~1.39.0"
QuantumCumulants = "~0.2.22"
QuantumOptics = "~1.0.14"
"""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "0bfd3fdc22963d4dfbac28e643945bbacb21e724"
...
git-tree-sha1 = "eefe93a09118448189ef5e89aa09f0a46467bca5"
...
[[deps.QuantumCumulants]]
deps = ["Combinatorics", "LaTeXStrings", "Latexify", "LinearAlgebra", "MacroTools", "ModelingToolkit", "QuantumOpticsBase", "SciMLBase", "SymbolicUtils", "Symbolics", "TermInterface"]
git-tree-sha1 = "4939bdeb27209646c501aa70750db947e0ce67f1"
uuid = "35bcea6d-e19f-57db-af74-8011de6c7255"
version = "0.2.22"

[[deps.QuantumInterface]]
deps = ["LinearAlgebra", "SparseArrays"]
git-tree-sha1 = "7fd2e76b69ce1bdb51274579acbcd715b1fe9d22"
...
git-tree-sha1 = "26933d20c753020dfb3e9b87c652a71d1f5d63f7"
uuid = "6e0679c1-51ea-5a7c-ac74-d61b76210b0c"
version = "1.0.14"

[[deps.QuantumOpticsBase]]
deps = ["Adapt", "FFTW", "FastExpm", "FillArrays", "LRUCache", "LinearAlgebra", "QuantumInterface", "Random", "SparseArrays", "Strided", "UnsafeArrays"]
git-tree-sha1 = "9230a6ce2f4a2513a0fe5e0b2b2bbfde8e6b2a46"
...
git-tree-sha1 = "137303f5e0a39f966b462c53ae2c5c6e34c4828b"
...
# ╠═00a9e7aa-c638-47f7-8452-ded62cff257b
# ╠═7caee069-cfd0-46cc-a643-6f7983da8077
# ╠═2bf136c5-49ec-4878-9f2d-cea85079c772
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
