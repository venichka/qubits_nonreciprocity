...
L_range=("3.31e-9 3.3e-9" "3.301e-9 3.3e-9" "3.3001e-9 3.3e-9")
N_exc=("2" "3" "5")
direction=("L" "R")
a_inc="3873e0"
t_max="1e1"
...
for L in "${L_range[@]}"; do
  for exc in "${N_exc[@]}"; do
    for dir in "${direction[@]}"; do
      ...
      if [ "$dir" == "L" ]; then
        power_lj_flag=""
        if [ "$L" == "3.31e-9 3.3e-9" ]; then
          power_lj_flag="--power_lj"
        fi
        python data_gen_power.py --L_js $L --N_exc $exc --t_max $t_max --tau_max $tau_max --nmax $nmax --time --nmax_time $nmax_time $power_lj_flag
      fi
    done
  done
done
