...
class _UnitedStrCaller(_UnitedFuncCaller):
    ...
    def __call__(self, t, args={}):
        ...
        g = now_args['g']
        w_ext = now_args['w_ext']
        w = now_args['w']
        a_L = now_args['a_L']
        t_j = now_args['t_j']
        a_R = now_args['a_R']
        out.append(-2.0*sqrt(0.5*g[0])*sqrt(w_ext/w[0]) *         (a_L*sin(w_ext*(t+t_j[0])) + a_R*sin(w_ext*(t-t_j[0]))) *         exp(-1j*w_ext*t))
        out.append(-2.0*sqrt(0.5*g[1])*sqrt(w_ext/w[1]) *         (a_L*sin(w_ext*(t+t_j[1])) + a_R*sin(w_ext*(t-t_j[1]))) *         exp(-1j*w_ext*t))
        out.append(-2.0*sqrt(0.5*g[0])*sqrt(w_ext/w[0]) *         (a_L*sin(w_ext*(t+t_j[0])) + a_R*sin(w_ext*(t-t_j[0]))) *         exp(1j*w_ext*t))
        out.append(-2.0*sqrt(0.5*g[1])*sqrt(w_ext/w[1]) *         (a_L*sin(w_ext*(t+t_j[1])) + a_R*sin(w_ext*(t-t_j[1]))) *         exp(1j*w_ext*t))
