...
\begin{document}
\title{Theory and Analysis of Nonreciprocity for Superconducting Qubits Coupled to a Waveguide}
...
\begin{IEEEkeywords}
Nonreciprocity, quantum correlations, subradiance, superconducting qubits.
\end{IEEEkeywords}
...
Note that further in the text, we use the notation where operators do not have the $\hat{...}$ symbol.
If we work with average values of operators, we always use $\langle \ldots \rangle$ notation.
Moreover, in this work, we do not consider the fine-tuning of the waveguide, such as its dispersion~\cite{he2018waveguide,he2020miniaturization}.
...
Collectively, these methods aim to enhance qubit performance by reducing sensitivity to noise sources and bolstering coherence and fidelity during qubit operations.
...
\begin{align} \label{eq:04}
    H_Q &= \hbar \sum_j \left( \omega_j a_j^\dag a_j + \frac{\alpha}{2 \hbar} a_j^\dag a_j^\dag a_j a_j \right), \\
...
The Hamiltonian (\ref{eq:04})--(8) describes the entire waveguide-qubit system. Still, using it for analytical and numerical calculations is cumbersome because it explicitly contains the infinite degrees of freedom associated with the waveguide modes.
...
We also assume that the waveguide field incident from two directions is monochromatic and is in a coherent state, with complex amplitudes $a_\mathrm{in}^{R/L}$ and driving frequency $\omega_d$.
Indeed the action of the incident field operator on the coherent state $|\beta\rangle$ is: $a_{R/L}(\omega) | \beta \rangle = \beta  \delta(\omega - \omega_d) | \beta \rangle$, where $\beta \equiv a_\mathrm{in}^{R/L} = |a_\mathrm{in}^{R/L}| e^{-i \omega_d \theta_{R/L}}$~\cite{scully1999quantum}.
...
\begin{align} \label{eq:06}
    H_\mathrm{eff} =& \hbar \sum_j \left( (\omega_j - \omega_d) a_j^\dag a_j + \frac{\alpha}{2 \hbar} a_j^\dag a_j^\dag a_j a_j \right) + \nonumber \\
...
As mentioned above and shown in Table~\ref{tab:01}, to break the inversion symmetry in the system, we consider a small detuning of qubit 1.
...
Fig.~\ref{fig:02}(c-e) show the eigenenergies of the effective Hamiltonian (\ref{eq:06}) also depending on the Josephson inductance of qubit 1, $L_J$.
...
\begin{equation}\label{eq:16}
    ...
    \end{aligned}
\end{equation}
Note that in the expressions above, we imply $a_\mathrm{in}^{R/L}$ as a product of the incident field amplitude and the identity operator in the Hilbert space of two qubits.
...
\begin{figure*}[t]
    ...
    \caption{(a) Incident power ($|a_\mathrm{in}|^2 / \bar{\gamma}$) dependence of transmission for R-propagating waves (red dashed lines) and L-propagating waves (blue solid lines); (b) diode efficiency; (c) the population of the dark state $|D\rangle$ and (d) the bright state $|B\rangle$ in steady-state for R- and L-propagating incident waves. All the plots are made for different numbers of levels in the qubits, namely, $N_l = 2, 3, 5$. The circle markers correspond to the solution without a rotating-wave approximation.}
    \label{fig:03}
\end{figure*}

\subsection{Diode Efficiency and System Populations}
...
The populations of the dark and bright states are shown in Fig.~\ref{fig:03}(c) and (d), correspondingly.
...
\subsection{Influence of Higher Levels}

Fig.~\ref{fig:03}(a) and (b) also demonstrate how the increase in the number of levels in the qubits affects the nonreciprocity effect, namely, transmission and reflection.
...
At the same time, populations of the dark and bright states do not change much with the increase in the qubits' energy levels, see Fig.~\ref{fig:03}(c) and (d).
...
Note that at the parameter range considered, RWA and non-RWA solutions have a good agreement and using RWA is justified; see Fig.~\ref{fig:03}.
...
The state $|G\rangle = |0,0\rangle$ is the system's ground state, and the state $|E\rangle  = |1,1\rangle$ is the state where both qubits have 1 excitation.
...
\subsection{Spectra and Coherence Functions}
...
These functions are plotted for the system with realistic parameters (see~\cite{rosario_hamann_nonreciprocity_2018} and Table~\ref{tab:01}).
...
The $g^{(2)}_L(\tau)$ functions, see~(\ref{eq:21}), are shown in Fig.~\ref{fig:06}(b).
At these parameters, Table~\ref{tab:01}, the second-order coherence functions for both transmitted and reflected signals vary strongly depending on the number of qubits' energy levels considered.
...
\begin{figure}[h]
    ...
    \label{fig:06_1}
\end{figure}
For the two-level case, we expect $g^{(2)}_{L, \mathrm{ref}}(0) = 3$ and $g^{(2)}_{L, \mathrm{trans}}(0) = 1.5$ and we can see this behaviour in Fig.~\ref{fig:06_1}.
...
We consider the system of two qubits in the transmon regime with parameters listed in Table~\ref{tab:01}.
...
Meanwhile, at smaller incident powers, it becomes negligible.
...
We use the real parameters of the qubits from ref.~\cite{rosario_hamann_nonreciprocity_2018} listed in Table~\ref{tab:01}.
...
Additionally, the second-order coherent functions for transmitted and reflected signals become less distinguishable and tend to coincide, as illustrated in Fig.~\ref{fig:06}.
...
These findings provide insights into the complexities of nonreciprocal quantum systems, with direct implications for practical quantum device engineering. 
...
% \begin{IEEEbiographynophoto}{Nikita Nefedkin}
% Biography text here without a photo.
% \end{IEEEbiographynophoto}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{fig_NN}}]{Nikita Nefedkin}
received his M.S. degree in Applied Mathematics and Physics followed by a Ph.D. in Theoretical Physics from the Moscow Institute of Physics and Technology, Russia, in 2017 and 2019, respectively. Since 2020, he has been serving as a Post-Doctoral Researcher at the Photonics Initiative within the City University of New York (CUNY) Advanced Science Research Center. His research interests include theoretical quantum optics, many-body physics, open quantum systems, nonlinear optics.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{fig_AA}}]{Andrea Al\`u}
is a Distinguished Professor at the City University of New York (CUNY), the Founding Director of the Photonics Initiative at the CUNY Advanced Science Research Center, the Einstein Professor of Physics at the CUNY Graduate Center, Professor of Electrical Engineering at the City College of New York, Senior Research Scientist and Adjunct Professor at the University of Texas at Austin. He received the Laurea, MS and PhD degrees from the University of Roma Tre, Rome, Italy, respectively in 2001, 2003 and 2007. After performing postdoctoral research at the University of Pennsylvania, in 2009 he joined the faculty of the University of Texas at Austin, where he was the Temple Foundation Endowed Professor until 2018. In 2015 he was the Royal Netherlands Academy of Arts and Sciences (KNAW) Visiting Professor at the AMOLF Institute in the Netherlands.

His research interests cover metamaterials and metasurfaces, applied electromagnetics, optics, nanophotonics, and acoustics. He is currently the Editor-in-Chief of Optical Materials Express and has been serving on the Editorial Board of Reviews of Electromagnetics, Physical Review B, Advanced Optical Materials, Laser and Photonics Reviews, New Journal of Physics, EPJ Applied Metamaterials and ISTE Metamaterials. He served as Associate Editor for IEEE Antennas and Wireless Propagation Letters, Optics Express, Applied Physics Letters, MDPI Materials, Scientific Reports, Metamaterials, and Advanced Electromagnetics. He has guest edited special issues for the IEEE Journal of Selected Topics in Quantum Electronics, Proceedings of IEEE (twice), IEEE Transactions on Antennas and Propagation, IEEE Antennas and Wireless Propagation Letters (twice), Nanophotonics, Journal of Optics, Journal of the Optical Society of America A and B, Photonics and Nanostructures - Fundamentals and Applications, Optics Communications, Metamaterials, and Sensors on a variety of topics involving metamaterials, plasmonics, optics and electromagnetic theory. 

Dr. Alù is currently the President of the Metamorphose Virtual Institute for Artificial Electromagnetic Materials and Metamaterials, and the Director of the Simons Collaboration on Extreme Wave Phenomena, an OSA Traveling Lecturer since 2010, and an IEEE AP-S Distinguished Lecturer since 2014. He is the chair of the IEEE joint AP-S, MTT-S, Photonics, NTC chapter in New York, and was a member of the Administrative Committee of the IEEE Antennas and Propagation Society, and the IEEE joint AP-S and MTT-S chapter chair for Central Texas. He is a Simons Investigator in Physics since 2016 and a full member of URSI, a Fellow of IEEE, NAI, AAAS, MRS, OSA, SPIE and APS, and a Highly Cited Researcher (Clarivate Web of Science) since 2017. Over the last few years, he has received several awards for his research activities, including the SPIE Mozi Award (2024), the IEEE Antennas and Propagation Society Distinguished Achievement Award (2023), the Brillouin Medal (2021), the Blavatnik National Award in Physical Sciences and Engineering (2021), the AAAFM Heeger Award (2021), the Dan Maydan Prize in Nanoscience (2021), the IEEE Kiyo Tomiyasu Award (2020), the DoD Vannevar Bush Faculty Fellowship (2019), IUMRS Young Researcher Award (2018), ICO Prize in Optics (2016), the inaugural MDPI Materials Young Investigator Award (2016), the Kavli Foundation Early Career Lectureship in Materials Science (2016), the inaugural ACS Photonics Young Investigator Award Lectureship (2016), the Edith and Peter O’Donnell Award in Engineering (2016), the NSF Alan T. Waterman Award (2015), the IEEE MTT Outstanding Young Engineer Award (2014), the OSA Adolph Lomb Medal (2013), the IUPAP Young Scientist Prize in Optics (2013), the Franco Strazzabosco Award for Young Engineers (2013), the SPIE Early Career Investigator Award (2012), the URSI Issac Koga Gold Medal (2011), an NSF CAREER award (2010), the AFOSR and the DTRA Young Investigator Awards (2010, 2011), Young Scientist Awards from URSI General Assembly (2005) and URSI Commission B (2010, 2007 and 2004). His students have also received several awards, including several student paper awards at IEEE Antennas and Propagation and URSI Symposia.
